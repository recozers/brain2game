...
export function createHiresBrain(assets, opts = {}) {
  ...
  renderer.setClearColor(0x000000, 0);   // transparent: the page's calm white shows through
  ...
  const material = new THREE.MeshPhysicalMaterial({
    color: 0xffffff, roughness: 0.58, metalness: 0.0, clearcoat: 0.3, clearcoatRoughness: 0.45, envMapIntensity: 0.3,
  });
...
vec3 hotRamp(float t) {
  vec3 c0 = vec3(1.0, 0.55, 0.22), c1 = vec3(0.95, 0.24, 0.20), c2 = vec3(0.72, 0.05, 0.16), c3 = vec3(0.40, 0.01, 0.12);
...
float shade = mix(1.0, 0.50, mix(0.65 * curvy, sulcal, uInflate));  // pial: curvature cue; inflated: full sulcal shading
vec3 tissue = vec3(0.97, 0.66, 0.67) * shade;                        // brain pink
...
vec3 hotEmissive = hot * actW * 0.12;                               // just enough that hot spots never fall into shadow`)
      .replace('#include <emissivemap_fragment>', `#include <emissivemap_fragment>
totalEmissiveRadiance += hotEmissive;`);
  };
  ...
  // ---- lights ---------------------------------------------------------------------------------
  scene.add(new THREE.HemisphereLight(0xffffff, 0xc9b4b4, 0.7));
  const key = new THREE.DirectionalLight(0xfff4ee, 1.15); key.position.set(-0.6, 0.9, 1.0);
  camera.add(key); scene.add(camera);   // key light rides with the camera
  const fill = new THREE.DirectionalLight(0xdfe6ff, 0.3); fill.position.set(1.0, -0.2, 1.0); scene.add(fill);
  const rim = new THREE.DirectionalLight(0xffe2c8, 0.25); rim.position.set(0.3, 0.5, 1.3); scene.add(rim);
